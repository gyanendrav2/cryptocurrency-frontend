...
import { Box, Flex, Grid } from "@react-cssx/core"
...
const Rechart = dynamic<{ pair: string }>(
  import("../Rechart").then((mod) => mod.Rechart),
...
export function Markets({ data }: IMarketsProps): ReactElement {
  ...
  const updateData = () => {
    axios
      .get("https://api.coinmetro.com/exchange/prices/")
  ...
  const filtered = marketsToShow.reduce((acc, market) => {
    const findLatest = markets?.latestPrices?.find(
      (item) => item.pair === marketMapping[market.name]
    )
  ...
  return (
    <Box cssx={{ pos: "relative", pb: 150, zIndex: 10 }}>
      ...
      >
        ...
        <Text richText cssx={{ mx: "auto", maxWidth: { desktop: 2 / 3 } }}>
          {data.market_content}
        </Text>
      </Container>
            ...
            <a href={entry.url} key={entry.symbol}>
              {entry.latest && entry.daily && (
                ...
                >
                  <Box cssx={{ py: 10 }}>
                    <img
                      src={`https://go.coinmetro.com/assets/img/assets/${entry.symbol}.svg`}
                      alt={entry.name}
                      width={40}
                      height={40}
                    />
                  ...
                  <Box
                    cssx={{ pr: 16, mr: "auto", color: entry.daily.delta > 0 ? "green" : "red" }}
                  >
                ...
                <Box cssx={{ bg: "grey.bg", mb: 10 }}>
                  <Container>
                    <Flex>
                      ...
                      <Flex
                        wrap="wrap"
                        cssx={{ flex: 1, py: 24, color: "purple.default" }}
                        align="center"
                      >
                        ...
                        <Flex direction="column" cssx={{ ml: "auto" }}>
                          ...
                          <Box
                            cssx={{
                              color: entry.daily.delta > 0 ? "green" : "red",
                              mt: 8,
                              ml: "auto",
                            }}
                          >
      ...
      <Container cssx={{ mt: 40, display: "flex" }}>
        <Button
          as="a"
          href="https://go.coinmetro.com/markets"
          variant="outline"
          cssx={{ mx: "auto" }}
        >
