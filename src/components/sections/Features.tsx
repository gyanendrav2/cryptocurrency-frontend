...
const Feature = ({ feature, invert, index }: any) => (
  <Container>
    <Flex wrap="wrap" direction={{ _: "column-reverse", tablet: "row" }}>
      <Box
        cssx={{
          p: { _: "125px 0 75px 0", tablet: "unset" },
          w: { _: "full", tablet: 1 / 2 },
          order: { tablet: invert ? 2 : 1 },
        }}
      >
        <Element
          as="img"
      ...
      >
        <Box cssx={{ pos: "relative" }}>
          ...
          <H3 cssx={{ pos: "absolute", bottom: 30 }} richText>
            {feature.title}
          </H3>
        </Box>
        <Text cssx={{ color: "grey.dark", mb: 20 }} richText>
          {feature.content}
        </Text>
...
export function Features({ data }: IFeaturesProps): ReactElement {
  return (
    ...
    >
      <H2 cssx={{ textAlign: "center", mb: 50 }} richText>
        {data.offers_title}
      </H2>
